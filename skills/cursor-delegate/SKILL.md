...
description: Use when you want to delegate a coding task to Cursor (the cursor-agent CLI) as a headless implementer while your coding agent orchestrates. The agent writes a self-contained brief, you pick the implementation model, Cursor does the edits, the agent reviews the diff (correctness + clean-code) and loops feedback back to Cursor until it passes, then you review and commit. Trigger phrases include "delegate this to Cursor", "have cursor-agent build", and "cursor-delegate".
---

# Cursor Delegate

Delegate a coding task to Cursor (the `cursor-agent` CLI) running headless. Your
coding agent orchestrates and reviews; `cursor-agent` implements; **you** commit.

## Roles (do not blur these)

- **The orchestrating agent (orchestrator + reviewer)** — Claude Code, OpenCode,
  Cursor, or similar: writes the brief, recommends a model, dispatches to
  `cursor-agent`, reviews the diff, writes feedback. It **never edits the
  implementer's code** and **never commits or pushes**.
- **`cursor-agent` (implementer):** makes all code edits. Never commits.
...
2. The orchestrator never edits the produced code — every fix goes back to `cursor-agent` as feedback.
3. Never commit, never push — present the diff and let the human land it.
4. Never put secrets in briefs; `cursor-agent` auth comes from environment variables.
...
2. **Write the brief.** Follow `references/writing-the-brief.md`. Save to
   `.cursor-delegate/brief-<timestamp>.md`.
...
4. **Pre-flight.** Run the checks in `references/cursor-commands.md`:
   - `cursor-agent` is on PATH (else refuse with the setup guidance in
     cursor-commands.md). If a dispatch later fails with an auth error, surface
     that guidance (run `cursor-agent login`, or set `CURSOR_API_KEY`).
   ...
   - If `git status --porcelain` is non-empty, offer to `git stash` unrelated
     changes so the captured diff is solely Cursor's work.

5. **Dispatch.** Run `CURSOR_DISPATCH` from `references/cursor-commands.md` with
   the chosen model and brief. Capture stdout, stderr, and exit code.

6. **Capture the diff.** `git diff` plus untracked files
   (`git status --porcelain`) to see exactly what `cursor-agent` changed.
...
9. **Feedback + re-dispatch.** Write a feedback file per
   `references/review-loop.md` to `.cursor-delegate/feedback-<n>.md`, then
   run `CURSOR_FEEDBACK`. Increment the iteration counter and loop to step 5.
...
## On `cursor-agent` failure

If `cursor-agent` exits non-zero, surface its stderr to the human and stop — do
not loop blindly on a failed run.
