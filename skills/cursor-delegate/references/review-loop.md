...
loop feedback back to `cursor-agent` or present to the human.
...
Write to `.cursor-delegate/feedback-<n>.md`:
...
Then re-dispatch with `CURSOR_FEEDBACK` (same model unless the human re-chooses).
