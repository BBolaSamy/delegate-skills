...
description: Use when you want to delegate a coding task to OpenCode (the opencode CLI) as a headless implementer while your coding agent orchestrates. The agent writes a self-contained brief, you pick the implementation model, OpenCode does the edits, the agent reviews the diff (correctness + clean-code) and loops feedback back to OpenCode until it passes, then you review and commit. Trigger phrases include "delegate this to OpenCode", "have opencode build", and "opencode-delegate".
---

# OpenCode Delegate

Delegate a coding task to OpenCode (the `opencode` CLI) running headless. Your
coding agent orchestrates and reviews; `opencode` implements; **you** commit.
...
- **The orchestrating agent (orchestrator + reviewer)** — Claude Code, OpenCode,
  ...
  `opencode`, reviews the diff, writes feedback. It **never edits the
  implementer's code** and **never commits or pushes**.
- **`opencode` (implementer):** makes all code edits. Never commits.
...
2. The orchestrator never edits the produced code — every fix goes back to `opencode` as feedback.
3. Never commit, never push — present the diff and let the human land it.
4. Never put secrets in briefs; `opencode` auth comes from environment variables.
...
2. **Write the brief.** Follow `references/writing-the-brief.md`. Save to
   `.opencode-delegate/brief-<timestamp>.md`.
...
4. **Pre-flight.** Run the checks in `references/opencode-commands.md`:
   - `opencode` is on PATH (else refuse with the setup guidance in
     opencode-commands.md). If a dispatch later fails with an auth error,
     surface that guidance (run `opencode auth login`, or set the provider key
   ...
   - If `git status --porcelain` is non-empty, offer to `git stash` unrelated
     changes so the captured diff is solely OpenCode's work.

5. **Dispatch.** Run `OPENCODE_DISPATCH` from `references/opencode-commands.md`
   with the chosen model and brief. Capture stdout, stderr, and exit code.

6. **Capture the diff.** `git diff` plus untracked files
   (`git status --porcelain`) to see exactly what `opencode` changed.
...
9. **Feedback + re-dispatch.** Write a feedback file per
   `references/review-loop.md` to `.opencode-delegate/feedback-<n>.md`, then
   run `OPENCODE_FEEDBACK`. Increment the iteration counter and loop to step 5.
...
If `opencode` exits non-zero, surface its stderr to the human and stop — do not
loop blindly on a failed run.
