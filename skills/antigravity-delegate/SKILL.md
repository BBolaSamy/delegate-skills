...
Delegate a coding task to the Antigravity CLI (`agy`) running headless. Claude
orchestrates and reviews; `agy` implements; **you** commit.

## Roles (do not blur these)

- **Claude (orchestrator + reviewer):** writes the brief, recommends a model,
  dispatches to `agy`, reviews the diff, writes feedback. Claude **never edits
  the implementer's code** and **never commits or pushes**.
...
2. Claude never edits the produced code — every fix goes back to `agy` as feedback.
...
7. **Review.** Run two passes on the diff:
   - **code-review** — correctness, bugs, security (use the `/code-review`
     capability / code-reviewer).
   - **clean-code** — invoke the `clean-code-guard` skill.
