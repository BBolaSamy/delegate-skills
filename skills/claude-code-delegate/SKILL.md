...
description: Use when you want to delegate a coding task to Claude Code (the claude CLI) as a headless implementer while your coding agent orchestrates. The agent writes a self-contained brief, you pick the implementation model, Claude Code does the edits, the agent reviews the diff (correctness + clean-code) and loops feedback back to Claude Code until it passes, then you review and commit. Trigger phrases include "delegate this to Claude Code", "have claude build", and "claude-code-delegate".
---

# Claude Code Delegate

Delegate a coding task to Claude Code (the `claude` CLI) running headless. Your
coding agent orchestrates and reviews; `claude` implements; **you** commit.
...
- **The orchestrating agent (orchestrator + reviewer)** — Claude Code, OpenCode,
  ...
  `claude`, reviews the diff, writes feedback. It **never edits the
  implementer's code** and **never commits or pushes**.
- **`claude` (implementer):** makes all code edits. Never commits.
...
2. The orchestrator never edits the produced code — every fix goes back to `claude` as feedback.
3. Never commit, never push — present the diff and let the human land it.
4. Never put secrets in briefs; `claude` auth comes from environment variables.
...
2. **Write the brief.** Follow `references/writing-the-brief.md`. Save to
   `.claude-code-delegate/brief-<timestamp>.md`.
...
4. **Pre-flight.** Run the checks in `references/claude-commands.md`:
   - `claude` is on PATH (else refuse with the setup guidance in
     claude-commands.md). If a dispatch later fails with an auth error, surface
     that guidance (log in to Claude, or set `ANTHROPIC_API_KEY`).
   ...
   - If `git status --porcelain` is non-empty, offer to `git stash` unrelated
     changes so the captured diff is solely Claude Code's work.

5. **Dispatch.** Run `CLAUDE_DISPATCH` from `references/claude-commands.md` with
   the chosen model and brief. Capture stdout, stderr, and exit code.

6. **Capture the diff.** `git diff` plus untracked files
   (`git status --porcelain`) to see exactly what `claude` changed.
...
9. **Feedback + re-dispatch.** Write a feedback file per
   `references/review-loop.md` to `.claude-code-delegate/feedback-<n>.md`, then
   run `CLAUDE_FEEDBACK`. Increment the iteration counter and loop to step 5.
...
If `claude` exits non-zero, surface its stderr to the human and stop — do not
