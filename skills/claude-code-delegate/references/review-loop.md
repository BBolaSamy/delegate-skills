...
loop feedback back to `claude` or present to the human.
...
Write to `.claude-code-delegate/feedback-<n>.md`:
...
Then re-dispatch with `CLAUDE_FEEDBACK` (same model unless the human re-chooses).
