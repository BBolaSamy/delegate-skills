...
**delegate-agent** is a family of skills that each delegate a coding task to a headless coding-agent CLI. Your orchestrating agent (Claude Code, OpenCode, Cursor, …) writes a self-contained brief, you pick the model, the implementer CLI does the edits, your agent reviews the diff and loops feedback back until it passes — and **you** commit. There is one skill per implementer:
...
```bash
npx skills add BBolaSamy/delegate-agent --skill antigravity-delegate
npx skills add BBolaSamy/delegate-agent --skill opencode-delegate
npx skills add BBolaSamy/delegate-agent --skill cursor-delegate
npx skills add BBolaSamy/delegate-agent --skill claude-code-delegate
```
